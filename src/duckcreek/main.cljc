(ns duckcreek.main
  "Kotodama WASM entrypoint for the Duckcreek clean-room actor (L4) — Clojure port.
  ...
  facts (`duckcreek.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "duckcreek")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"duckcreek.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "duckcreek-compat" :tier tier :entities entities} 200])
...
(defn start! [] :duckcreek-compat/ready)
